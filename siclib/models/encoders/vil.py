...
class ViL(BaseModel):
    ...
    def _init(
        ...
        stride = None
        legacy_norm = conf.legacy_norm
        conv_kind = conf.conv_kind
        conv_kernel_size = conf.conv_kernel_size
        proj_bias = conf.proj_bias
        norm_bias = conf.norm_bias
        init_weights = "original"
    ...
    def _forward(self, data):
        ...
        # 还原原始形状
        x = einops.rearrange(x, "b ( h w ) d -> b d h w", h=origin_shape[1], w=origin_shape[2])
    ...
    def flexible_load(self, state_dict):
        ...
        # TODO: 预训练模型和实际使用的模型不一致
        state_dict.pop("pos_embed.embed")
        # if self.legacy_norm:
