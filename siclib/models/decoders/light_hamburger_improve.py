...
class LightHamHead(BaseModel):
    ...
    def _init(self, conf):
        ...
        if self.with_ll:
            ...
            self.ll_fusion1 = FeatureFusionBlock(self.out_channels, upsample=False)
            self.ll_fusion2 = FeatureFusionBlock(self.out_channels, upsample=False)
    ...
    def _forward(self, features):
        ...
        if self.with_ll:
            ...
            for i, ax in enumerate(axes_flat):
                ax.imshow(feats_ll[0][i].detach().cpu() * 10, cmap="gray")
                ax.axis('off')
            plt.show()
