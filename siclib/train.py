...
def get_lr_scheduler(optimizer, conf):
    ...
    if conf.type not in ["factor", "exp", None]:
        ...
        if "lr_lambda" in conf.options:
            print(conf.options["lr_lambda"])
            conf.options["lr_lambda"] = lambda epoch: epoch // 2
...
def training(rank, conf, output_dir, args):
    if args.restore:
        ...
        init_cp = torch.load(str(init_cp), map_location="cpu", weights_only=False)
        ...
        best_cp = torch.load(str(best_cp), map_location="cpu", weights_only=False)
